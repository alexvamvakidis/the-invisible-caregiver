...
TRACKED_SENSORS = {
    ...
    "bathroom_water_flow":        False,  # False = water is running
    "kitchen_faucet":             False,  # False = water is running
...
def compute_carryover(summary: dict, window_end_ts: float) -> dict:
    """
    Extract sensors still in an active/concerning state at the end of the current window.
    Returns a carryover dict to be saved and injected into the next audit.
    """
    ...
    return {
        ...
        "window_end_local": datetime.fromtimestamp(window_end_ts).strftime("%Y-%m-%d %H:%M:%S"),
...
def format_carryover_section(carryover: dict | None) -> str:
    ...
    lines = [
        ...
        f"Previous window ended: {carryover.get('window_end_local', 'unknown')}",
        "",
